...
True when the session is loading for client side apps.
...
<Aside type="note">
  ...
  You can also extract the union type of your configured providers using the `ProviderIds` utility:
  
  ```ts
  import type { ProviderIds } from '@rttnd/gau'
  import type { Auth } from './server/auth'

  export type Provider = ProviderIds<Auth> // 'github' | 'google'
  ```
</Aside>
...
A fetch wrapper for making authenticated API calls to your backend.
...
**What it handles:**
- Adds `Authorization: Bearer <token>` if a token is stored (Tauri/mobile)
- Falls back to `credentials: 'include'` for cookie-based auth (web)
- Auto-stores refreshed tokens from the `X-Refreshed-Token` header

<Aside type="tip">
  For pure web apps using cookies, you don't need `fetch` from useAuth, regular `fetch` with `credentials: 'include'` works fine. This is mainly useful for Tauri/mobile apps that use bearer tokens.
</Aside>

See the [Middleware](/guides/middleware) guide for setting up automatic session refresh on the server.
