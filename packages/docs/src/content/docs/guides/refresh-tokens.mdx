---
title: Refresh Tokens
description: Understanding provider refresh tokens and session duration in gau.
---

import { Steps, Tabs, TabItem } from '@astrojs/starlight/components'
import Aside from 'starlight-plugin-icons/components/Aside.astro'

This guide covers two distinct concepts: **provider refresh tokens** (for calling external APIs) and **session refresh tokens** (for extending user sessions).

## Provider Refresh Tokens

`gau` uses its own JWT for sessions and does not require OAuth refresh tokens to keep a user signed in. However, when you need to call a provider API (Google, Microsoft), `gau` can store provider access/refresh tokens and automatically refresh/rotate them for you.

### What are provider refresh tokens?

Provider refresh tokens are long-lived credentials issued by OAuth providers (Google, Microsoft, etc.) to obtain new, short-lived access tokens without user re-authentication. These are used to call the provider's APIs on behalf of the user—not for `gau` sessions.

<Aside type="note">
  Some providers (e.g., GitHub) often do not issue refresh tokens. In this case, the `refreshToken` column will remain `null`.
</Aside>

### How gau stores tokens

During the OAuth callback, `gau` saves provider tokens into the `accounts` table via your adapter:

- `accessToken`, `refreshToken`, `expiresAt`, `tokenType`, `scope`, `idToken` (when available)
- On subsequent sign-ins, these values are updated for already-linked accounts.

### Automatic Refresh & Rotation

`gau` exposes a server-side helper to retrieve a valid provider access token, refreshing and rotating as needed:

```ts title="server/usage.ts"
const result = await auth.getAccessToken(userId, 'google')
if (!result)
  throw new Error('No access token available')

const { accessToken } = result
// Use accessToken to call Google APIs...
```

Behavior:
- If the stored access token is still valid, it's returned as-is.
- If it's expired and a `refreshToken` exists (and the provider supports it), `gau` calls the provider's token endpoint.
- If a new refresh token is returned, `gau` rotates it and persists the change via your adapter.

## Session Refresh Tokens

<Aside type="caution" title="Not Implemented">
  `gau` does not currently implement session refresh tokens. Sessions are managed using simple expiring JWTs.

  **For longer sessions**, increase [`jwt.ttl`](/guides/configuration#jwtttl) in your config. The default is 7 days; you can safely set it to 30 days or more for most apps.

  **Session refresh tokens** do have benefits: sliding session expiration, logout all devices, instant ban. If this is important to you, [open an issue](https://github.com/Rettend/gau/issues).
</Aside>
