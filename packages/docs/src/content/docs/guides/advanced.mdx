...
```ts
const refreshed = await auth.refreshSession(existingToken, {
...
3. ##### Keep Sessions Alive (Optional)
    ...
    Use `refreshSession()` to extend the session on each visit. This issues a new token with fresh TTL while preserving custom claims:
    ...
    export const handle = async ({ event, resolve }) => {
      ...
      if (token) {
        // Only refresh if past 50% of TTL
        const refreshed = await auth.refreshSession(token, { threshold: 0.5 })
        
        if (refreshed) {
          event.cookies.set(refreshed.cookieName, refreshed.token, {
            path: '/',
