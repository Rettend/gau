...
5. ##### Configure the Frontend
   ...
   The frontend needs to be configured to build as a static single-page app for Tauri, listen for the deep link event, and handle the sign-in flow. If you are building a full-stack app, you can still host your backend, and the Tauri app will communicate with it as well.
    ...
    <Tabs syncKey="framework">
      ...
      <TabItem label="SolidStart">
        Solid 2 does not globally patch `fetch`. Configure its server-function transport once in the client bootstrap, before hydration. The server-function endpoint is explicit and separate from Gau's auth `baseUrl`.

        The pinned start-mode plugin has no client-only setup hook for generated SSR entries. If you author `src/entry-client.tsx`, also author the matching `src/entry-server.tsx`; a lone client entry is not a valid SSR entry setup. These two files reproduce the generated entry behavior while adding the transport configuration.
        ...
        The helper adds the current bearer token without replacing an existing `Authorization` header and observes `X-Refreshed-Token`. For a cross-origin endpoint, allow the Tauri webview origin, `Authorization`, and Solid's server-function headers in CORS. Also expose `X-Refreshed-Token` through `Access-Control-Expose-Headers`; otherwise browser fetch hides it from the helper.

        Applications that do not need custom client bootstrap code should keep the generated-entry architecture. Legacy SolidStart on Solid 1 keeps its existing automatic bridge: set `SERVER_BASE_URL` to the independent server-function host and optional base path. The explicit config above is for Solid 2 start mode and is composable with other transport hooks.
      </TabItem>
    </Tabs>
